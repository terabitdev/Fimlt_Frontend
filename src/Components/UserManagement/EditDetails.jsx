...
function RoomMeasurementInterface({ projectData, onUpdateDetails, onUpdatePreview, isUpdating }) {
  ...
  const [editingField, setEditingField] = useState(null); // { type: 'category'|'subCategory'|'name'|'dimension', index: number }
  ...
  const handleEditClick = (type, index = 0) => {
    ...
    switch (type) {
      case 'category':
        currentValue = roomDetails.category || "";
        break;
      case 'subCategory':
        currentValue = roomDetails.subCategory || "";
        break;
      case 'name':
  ...
  const handleSave = () => {
    ...
    let updatedSubCategory = projectData.subCategory;
    ...
    switch (type) {
      case 'category':
        ...
        break;
      case 'subCategory':
        updatedSubCategory = editValue;
        break;
      case 'name':
        // Update the name field in the roomData object at index (remember we skip index 0)
        const actualIndex = index + 1; // Add 1 because we skip index 0
        if (updatedRoomData[actualIndex]) {
          updatedRoomData[actualIndex] = {
            ...updatedRoomData[actualIndex],
            name: editValue
          };
        }
        break;
      case 'dimension':
        // Update the dimensions field in the roomData object at index (remember we skip index 0)
        const actualDimIndex = index + 1; // Add 1 because we skip index 0
        if (updatedRoomData[actualDimIndex]) {
          updatedRoomData[actualDimIndex] = {
            ...updatedRoomData[actualDimIndex],
                ...
                {roomDetails.subCategory && (
                  <>
                    <tr>
                      ...
                      <td className="text-right py-1 px-2">
                        <button
                          onClick={() => handleEditClick('subCategory', 0)}
                          className="p-1"
                    ...
                    <tr>
                      <td colSpan="2" className="text-left py-1 px-2 font-[400] text-[#090D00]">
                        {editingField?.type === 'subCategory' && editingField?.index === 0 ? (
                          <input
                            type="text"
                            value={editValue}
                            onChange={handleInputChange}
  ...
  const handleUpdateDetails = async (updatedProject) => {
    ...
    try {
      ...
      const updateData = {
        ...
        subCategory: updatedProject.subCategory,
