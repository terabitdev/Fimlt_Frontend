...
//     if (!linePath) return '';
//     return `${linePath} L1000000,10 L0,100000 Z`;
//   };
...
//             {/* Chart area */}
//             <div className="absolute left-12  right-0 top- bottom-6 overflow-hidden">
    ...
    try {
      ...
      usersSnapshot.forEach((doc) => {
        ...
        if (userData.creationDate) {
          ...
          const monthYear = `${creationDate.getFullYear()}-${(creationDate.getMonth() + 1).toString().padStart(2, '0')}`;
          monthlyUsers[monthYear] = (monthlyUsers[monthYear] || 0) + 1;
        }
      });
      ...
      if (userIds.length > 0) {
        ...
        for (let i = 0; i < userIds.length; i += batchSize) {
          ...
          scansSnapshot.forEach((doc) => {
            ...
            const scanData = doc.data();
            if (scanData.timestamp) {
              // Properly handle Firebase timestamp for scan date
              const scanDate = scanData.timestamp.toDate ? 
                scanData.timestamp.toDate() : new Date(scanData.timestamp);
          ...
          <div className="h-64 relative">
            ...
            <div className="absolute left-12 right-0 bottom-0 flex justify-between text-gray-500 text-sm">
              {chartData.map((point, index) => (
                <div 
                  key={index} 
                  className={`${point.isSelected ? 'font-bold text-[#1E3A5F]' : ''}`}
                >
                  {point.month}
                </div>
              ))}
